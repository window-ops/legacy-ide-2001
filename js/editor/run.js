...
function runCode() {
  if (!STATE.activeFile) {
    showToast("Deschideți un fișier întâi.");
    return;
  }
  ...
  if (fx.programBroken) {
    output.innerHTML =
      '<div class="traceback"><span class="tb-title">Kernel panic satiric</span>Patch-urile au destabilizat executabilul. Unele module nu mai pornesc.</div>';
    $("outputStatus").innerHTML =
      '<span style="color: var(--error);">&#9679; Program broken</span>';
  ...
  if (lang.rules === "md") {
    if (!STATE.appliance.services.markdown) {
      output.innerHTML =
        '<div class="traceback"><span class="tb-title">Markdown indisponibil</span>Backend-ul QNX nu are parser Markdown activ. Activați-l runtime din shell cu <code>mdpatch</code> (sau <code>markdown-enable</code>).</div>';
      $("outputStatus").innerHTML =
        '<span style="color: var(--warning);">&#9679; Parser markdown inactiv</span>';
      return;
    }
    ...
    md = md
      ...
      .replace(/\n/g, "<br>");
    renderPreview(
      '<html><body style="font-family:Tahoma,Arial,sans-serif;padding:16px;">' +
        md +
        "</body></html>",
    );
    output.innerHTML =
      '<div class="success-box"><b>&#10003; Markdown randat.</b>\nParser activ în backend-ul QNX.</div>';
    $("outputStatus").innerHTML =
      '<span style="color: var(--success);">&#9679; Executat</span>';
    return;
  }
  if (!lang.rules) {
    ...
    box.innerHTML =
      "<b>&#10003; Fișier neverificat.</b>\nFișierele de tip <code>" +
      escapeHtml(node.name.split(".").pop()) +
      "</code> nu sunt acoperite de programa 2001 TIC.";
    output.appendChild(box);
    $("outputStatus").innerHTML =
      '<span style="color: var(--fg-dim);">&#9679; Neexecutat</span>';
    return;
  }
  ...
  if (backendBlocking) {
    ...
    msg.innerHTML =
      ...
      (hits.length > 0
        ? "- " +
          hits.length +
          " încălcare(i) raportate de programa analitică, agregate mai jos.<br>"
        : "") +
    ...
    if (hits.length === 0) {
      $("outputStatus").innerHTML =
        '<span style="color: var(--error);">&#9679; Blocat de backend</span>';
      return;
    }
  }
    ...
    hits = [
      {
        ...
        col: 1,
        rule: {
          label: "forced_traceback",
          year: 2001,
          standard: "Patched branch",
          remedy:
            "Revocați patch-ul NOP pe jz .clean pentru comportament normal.",
        },
      },
    ];
  }
  ...
  var curriculumNb =
    ...
    !STATE.serverBackendOnline;
  if (hits.length > 0) {
    if (fx.suppressTraceback) {
      output.innerHTML =
        '<div class="traceback"><span class="tb-title">Traceback incomplet</span>emit_traceback() a fost NOP-at. Diagnosticul este parțial corupt.</div>';
    }
    ...
    for (var i = 0; i < hits.length; i++) {
      ...
      pre.innerHTML =
        ...
        rowTitle +
        "</span>" +
        formatTraceback(hits[i], node.name, {
          continueExecution: curriculumNb,
        });
    ...
    if (curriculumNb) {
      ...
      sumNb.innerHTML =
        "<b>&#9888; Rezultat parțial.</b> " +
        (ro
          ? "<b>" +
            hits.length +
            "</b> încălcare(i) raportate; execuția continuă din cauza patch-ului pe returul din <code>check_curriculum</code>."
          : "<b>" +
            hits.length +
            "</b> violation(s) reported; execution continues due to patched <code>check_curriculum</code> return.");
      output.appendChild(sumNb);
    } else {
      var sum = document.createElement("div");
      sum.className = "traceback";
      sum.innerHTML =
        '<span class="tb-title">Rezultat final</span>Compilare eșuată. <b>' +
        hits.length +
        "</b> încălcare(i).";
      output.appendChild(sum);
      $("outputStatus").innerHTML =
        '<span style="color: var(--error);">&#9679; Compilare eșuată</span>';
  ...
  if (hits.length === 0 || curriculumNb) {
    ...
    if (resolved.missing.length) {
      ...
      warn.innerHTML =
        '<span class="tb-title">Referințe lipsă</span>Nu au putut fi rezolvate: <code>' +
        escapeHtml(resolved.missing.join(", ")) +
        "</code>.";
    ...
    if (lang.rules === "html") {
      ...
      if (STATE.prefs.antiHang === false) {
        ...
        var wrapped =
          "<html><head><title>Iesire</title><style>body{font-family:Cantarell,sans-serif;padding:16px;}pre{white-space:pre-wrap;}</style></head><body>" +
    ...
    if (curriculumNb && hits.length > 0) {
      $("outputStatus").innerHTML =
        '<span style="color: var(--warning);">&#9679; Executat cu avertismente</span>';
      showToast(
        STATE.prefs.romanian
          ? "Executat cu avertismente curriculare."
          : "Executed with curriculum warnings.",
      );
    } else {
      $("outputStatus").innerHTML =
        '<span style="color: var(--success);">&#9679; Executat</span>';
...
function sanitizePreviewHtml(html) {
  ...
  doc
    .querySelectorAll("script,iframe,object,embed,frame,frameset")
    .forEach(function (el) {
      el.remove();
    });
...
(function bindToolbarOnce() {
  ...
  if (lintBtn)
    lintBtn.addEventListener("click", function () {
      ...
      if (backendBlocking) {
        ...
        for (var k = 0; k < hits.length; k++) {
          ...
          pre2.innerHTML =
            ...
            t2 +
            "</span>" +
            formatTraceback(hits[k], node.name, {});
      ...
      if (hits.length === 0) {
        output.innerHTML =
          '<div class="success-box"><b>&#10003; Verificare completă.</b>\n0 încălcări detectate.</div>';
        ...
        for (var j = 0; j < hits.length; j++) {
          ...
          pre.innerHTML =
            ...
            t +
            "</span>" +
            formatTraceback(hits[j], node.name, { continueExecution: lintNb });
          output.appendChild(pre);
        }
  ...
  if (clearBtn)
    clearBtn.addEventListener("click", function () {
      $("output").innerHTML =
        '<span class="empty">&gt; Consola a fost curățată.</span>';
  ...
  document.querySelectorAll(".output-tab").forEach(function (el) {
    el.addEventListener("click", function () {
      switchOutputTab(this.getAttribute("data-otab"));
    });
  });
})();
